...
if [ -z "$S6_VERSION" ] || [ "$S6_VERSION" = "latest" ]; then
    S6_VERSION="$(url-fetch "https://api.github.com/repos/just-containers/s6-overlay/releases/latest" | grep 'tag_name' | cut -d\" -f4)"
...
url-fetch "$S6_TAR_URL" > /tmp/s6-overlay.tar.gz
